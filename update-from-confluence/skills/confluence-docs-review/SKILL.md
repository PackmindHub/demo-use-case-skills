---
name: confluence-docs-review
description: Manually triggered skill that browses Confluence documentation using Atlassian MCP tools, extracts coding practices and technical knowledge, classifies them for playbook relevance, and auto-invokes packmind-update-playbook with structured findings. Use when wanting to mine Confluence pages for conventions, architectural decisions, best practices, or technical knowledge that should be captured in the Packmind playbook. Supports non-interactive mode via `CI=true` or `--non-interactive` for scheduled runs.
---

# Confluence Documentation Review

Browse Confluence documentation to extract coding practices, technical knowledge, and conventions, classify them for playbook relevance, and feed actionable findings into the `packmind-update-playbook` workflow.

**Workflow: Phase 0 → 1 → 2 → 3 → 4 → 5. Follow every phase in order.**

> **Prerequisite**: The Atlassian Rovo MCP server must be available. Load `references/confluence-mcp-tools.md` for tool signatures and parameters.
...
3. **Parse optional arguments**:
   - `--url PAGE_URL` — fetch a specific Confluence page by URL
   - `--space SPACE_KEY` — browse pages within a specific space
   - `--query SEARCH_TERMS` — search Confluence for pages matching these terms

If neither condition is met, `non_interactive = false` (interactive mode — original behavior).

### Collect Parameters

1. **Get `cloudId`**: Use `mcp__confluence__getAccessibleAtlassianResources` to retrieve the cloud ID. This is required for all subsequent Confluence API calls. If multiple sites are returned, ask the user to choose (interactive) or use the first one (non-interactive).

2. **Gather search input**:
   - **Interactive**: Check if the user provided a URL, space, or query as arguments. If not, ask:
     > "How would you like to browse Confluence? You can:
     > 1. Provide a **page URL** to analyze a specific page
     > 2. Specify a **space** to browse pages within it
     > 3. Enter **search keywords** to find relevant pages
     >
     > What would you like to search for?"

     If the user provides a Confluence URL, extract the page ID from it. Common URL formats:
     - `https://<site>.atlassian.net/wiki/spaces/<SPACE>/pages/<PAGE_ID>/<title>`
     - `https://<site>.atlassian.net/wiki/x/<shortlink>`

   - **Non-interactive**: Use `--url`, `--space`, or `--query` value if provided. If **none** are provided, log a warning and exit gracefully:
     > "Non-interactive mode: no --url, --space, or --query provided. Exiting. Provide at least one parameter to specify what to search."

3. **Confirm** before proceeding:
   - **Interactive**: Display and **BLOCK** until the user confirms:
     - URL mode: > "Fetching Confluence page at **URL**. Proceed?"
     - Space mode: > "Browsing pages in space **SPACE_KEY**. Proceed?"
     - Query mode: > "Searching Confluence for **'SEARCH_TERMS'**. Proceed?"
...
Use `mcp__confluence__getConfluencePage` with `contentFormat: "markdown"` to fetch the page content directly.

- If the page is not found or inaccessible, inform the user and stop.
- Result: a single page to analyze.

### Space Mode (browse space)

1. If `--space` provides a space key, first use `mcp__confluence__getConfluenceSpaces` with the `keys` parameter to resolve the space ID.
2. Use `mcp__confluence__getPagesInConfluenceSpace` to list pages in the space, sorted by `-modified-date`.
3. **Interactive**: Display the page list and ask the user to select which pages to analyze (or "all").
4. **Non-interactive**: Take all pages (capped at 50).
5. For each selected page, use `mcp__confluence__getConfluencePage` with `contentFormat: "markdown"` to fetch full content.

### Query Mode (search)

1. Use `mcp__confluence__search` with the user's query to find relevant pages.
2. From the results, identify Confluence page ARIs (filter out Jira issue results).
3. Use `mcp__confluence__fetch` or `mcp__confluence__getConfluencePage` to retrieve full content for each matched page.
...
- Use the markdown body content from each page.
- Record metadata for each page:
  - **Page title**
  - **Page ID**
  - **Space** (key and name)
  - **Last modified date**
  - **Full Confluence URL** (mandatory) — construct from the API response `_links.webui` field combined with the site base URL (e.g., `https://<site>.atlassian.net/wiki<webui_path>`). If the URL was provided as input, use it directly. **Every page in the report MUST have a navigable URL** — this is critical for source attribution in `packmind-update-playbook`.
...
- **Non-interactive**: `.claude/reports/confluence-review-findings-YYYY-MM-DD.md` (using today's date, timestamped for CI artifact upload)
...
```markdown
# Confluence Documentation Review Findings Report

**Source**: <URL | Space KEY | Search "query">
...
| Page | Space | Last Modified | Summary |
|------|-------|---------------|---------|
| [Page Title](url) | SPACE | YYYY-MM-DD | Summary of page content |
| [Page Title](url) | SPACE | YYYY-MM-DD | Summary of page content |
...
**Every page reference in the report MUST include the full, navigable Confluence URL** (e.g., `https://mysite.atlassian.net/wiki/spaces/ENG/pages/123456789/Page+Title`). This is critical because `packmind-update-playbook` extracts source URLs from the findings report for its `packmind-cli diff --submit -m` source attribution. The expected attribution format is:

```
<topic>: <summary> (source: Confluence <Page Title> https://mysite.atlassian.net/wiki/spaces/ENG/pages/123456789)
```

If a URL cannot be constructed (e.g., missing `_links.webui`), fall back to `https://<site>.atlassian.net/wiki/pages/<pageId>`.
...
2. Ask the user:
   > "Found **N recurring themes** and **M individual findings**. Review the full report at `.claude/tmp/confluence-review-findings.md`. Proceed to update the playbook with these findings?"
...
3. **On confirm**: Invoke the `packmind-update-playbook` skill with the full report content as the intent. This maps to **Case B (explicit intent)** of that skill's Phase 0. Frame the intent as:
   > "Update the Packmind playbook based on the following Confluence documentation review findings report: <full report content>"

4. **On decline**: Inform the user the report is available at `.claude/tmp/confluence-review-findings.md` for manual review.

### Non-interactive mode

1. Display a summary (same as interactive: themes count, findings count, suggested actions).
2. Log the report output path:
   > "Report written to `.claude/reports/confluence-review-findings-YYYY-MM-DD.md`."
...
### references/
- `confluence-mcp-tools.md` — Atlassian Confluence MCP tool signatures and parameters used by this skill
